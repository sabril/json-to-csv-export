...
var csvDownload = function csvDownload(data, name, delimiter, stringify) {
  var items = data;
  var filename = name || 'export.csv';
  var d = delimiter || ',';
  var json_stringify = stringify;
  ...
  var csv = items.map(function (row) {
    return header.map(function (fieldName) {
      return json_stringify ? JSON.stringify(row[fieldName] || '') : row[fieldName] || '';
    }).join(d);
  });
